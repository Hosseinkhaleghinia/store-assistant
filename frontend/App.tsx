...
import { AudioRecorder } from './components/AudioRecorder';
import { Message, MessageRole } from './types';
import { sendTextMessage, sendVoiceMessage, API_BASE_URL } from './services/api';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const data = await sendTextMessage(userMsg.content, threadId, isTtsEnabled);
      
      const audioUrl = data.audio_url ? `${API_BASE_URL}${data.audio_url}` : undefined;
  ...
  const handleVoiceRecording = async (blob: Blob) => {
    const userMsg: Message = {
      id: generateId(),
      role: MessageRole.USER,
      content: '🎤 پیام صوتی در حال پردازش...',
      timestamp: Date.now()
    };
    ...
    try {
      const data = await sendVoiceMessage(blob, threadId, isTtsEnabled);
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        ...
        <div className="flex gap-2">
          <button 
            ...
            title={isTtsEnabled ? "خاموش کردن صدا" : "روشن کردن صدا"}
          >
